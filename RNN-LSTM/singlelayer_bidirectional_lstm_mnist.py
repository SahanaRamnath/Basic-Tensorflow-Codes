'''
------------------------------------------------------------
RNN-(SINGLE LAYERED) BIDIRECTIONAL LSTM IMPLEMENTED ON MNIST
------------------------------------------------------------
'''
...
# Bidirectional LSTM trains two LSTMs rather than one on the input sequence(on the input sequence and its reverse)
...
W=tf.Variable(tf.random_normal([2*h_len,num_classes],stddev=0.1))
...
lstm_cell_fw=rnn.BasicLSTMCell(num_units=h_len,forget_bias=1.0)
lstm_cell_bw=rnn.BasicLSTMCell(num_units=h_len,forget_bias=1.0)

#output and cell state of the LSTM
lstm_output,state_fw,state_bw=rnn.static_bidirectional_rnn(cell_fw=lstm_cell_fw,cell_bw=lstm_cell_bw,inputs=X_modified,dtype=tf.float32)
